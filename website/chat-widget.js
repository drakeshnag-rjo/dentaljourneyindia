(function () {
  ...
  style.textContent = `
    #dji-chat-btn{position:fixed;bottom:24px;right:24px;z-index:9999;width:60px;height:60px;border-radius:50%;background:linear-gradient(135deg,#0FA48A,#0B8570);border:none;cursor:pointer;box-shadow:0 4px 20px rgba(15,164,138,0.4);display:flex;align-items:center;justify-content:center;transition:transform .3s,box-shadow .3s}
    #dji-chat-btn:hover{transform:scale(1.1);box-shadow:0 6px 28px rgba(15,164,138,0.5)}
    #dji-chat-btn svg{width:28px;height:28px;fill:white}
    #dji-chat-window{position:fixed;bottom:96px;right:24px;z-index:9998;width:380px;max-width:calc(100vw - 48px);height:520px;max-height:70vh;background:#0F2E29;border:1px solid rgba(15,164,138,0.2);border-radius:16px;display:none;flex-direction:column;box-shadow:0 12px 48px rgba(0,0,0,0.4);overflow:hidden;font-family:'DM Sans',system-ui,sans-serif}
    #dji-chat-window.open{display:flex}
    #dji-chat-header{padding:16px 20px;background:linear-gradient(135deg,#143832,#1B4A42);border-bottom:1px solid rgba(15,164,138,0.15);display:flex;align-items:center;gap:12px}
    #dji-chat-header .avatar{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#0FA48A,#0B8570);display:flex;align-items:center;justify-content:center;font-size:18px}
    #dji-chat-header .info h4{color:#F6FBFA;font-size:14px;margin:0}
    #dji-chat-header .info span{color:#0FA48A;font-size:11px}
    #dji-chat-close{margin-left:auto;background:none;border:none;color:#7C948F;font-size:20px;cursor:pointer;padding:4px}
    ...
    #dji-chat-messages::-webkit-scrollbar-thumb{background:#1B4A42;border-radius:4px}
    .dji-msg{max-width:85%;padding:10px 14px;border-radius:12px;font-size:13.5px;line-height:1.5;color:#F6FBFA;animation:dji-fade .3s ease}
    .dji-msg.bot{align-self:flex-start;background:#1B4A42;border-bottom-left-radius:4px}
    .dji-msg.user{align-self:flex-end;background:linear-gradient(135deg,#0FA48A,#0B8570);color:#0F2E29;border-bottom-right-radius:4px}
    .dji-msg.typing{align-self:flex-start;background:#1B4A42;border-bottom-left-radius:4px}
    .dji-msg.typing .dots span{display:inline-block;width:6px;height:6px;border-radius:50%;background:#0FA48A;margin:0 2px;animation:dji-bounce 1.4s infinite}
    ...
    #dji-chat-input-area{padding:12px 16px;border-top:1px solid rgba(15,164,138,0.1);display:flex;gap:8px;background:#123531}
    #dji-chat-input{flex:1;background:#1B4A42;border:1px solid rgba(15,164,138,0.15);border-radius:24px;padding:10px 16px;color:#F6FBFA;font-size:13.5px;font-family:inherit;outline:none;transition:border-color .3s}
    #dji-chat-input:focus{border-color:#0FA48A}
    #dji-chat-input::placeholder{color:#54706B}
    #dji-chat-send{width:38px;height:38px;border-radius:50%;border:none;background:linear-gradient(135deg,#0FA48A,#0B8570);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform .2s}
    #dji-chat-send:hover{transform:scale(1.1)}
    #dji-chat-send:disabled{opacity:0.5;cursor:not-allowed;transform:none}
    #dji-chat-send svg{width:16px;height:16px;fill:#0F2E29}`;
  ...
  chatBtn.onclick=async()=>{isOpen=!isOpen;chatWindow.classList.toggle('open',isOpen);if(isOpen&&typeof gtag==='function')gtag('event','chat_open',{page_location:location.pathname});if(isOpen&&!sessionId)await startSession();if(isOpen)input.focus()};
  ...
  async function sendMessage(){const text=input.value.trim();if(!text)return;addMessage(text,'user');if(typeof gtag==='function'){gtag('event','chat_message_sent',{first_message:!firstMessageSent,page_location:location.pathname});firstMessageSent=true;}input.value='';sendBtn.disabled=true;const typing=addTyping();try{const r=await fetch(`${API_URL}/api/chat/message`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId,message:text})});const d=await r.json();typing.remove();addMessage(d.reply,'bot')}catch{typing.remove();addMessage("Sorry, having trouble connecting. Please try again!",'bot')}}
